...
String args = "&kind=d&csid=" + request.getParameter("csid");
if (cond == "") { 
	cond = "";
} else {
	args += "&cond=" + cond;
...
<tr><td>
<ul float="left" >
<li class="condition"><a href="list.product?kind=d&csid=<%=csId %>&cond=b">인기상품 순</a></li>
<li class="condition"><a href="list.product?kind=d&csid=<%=csId %>&cond=l">낮은가격 순</a></li>
<li class="condition"><a href="list.product?kind=d&csid=<%=csId %>&cond=h">높은가격 순</a></li>
</ul></td>
</tr>
<tr align="center" >
<% 
if (pdtList != null) {
	...
	<td width="33%" > <!-- 상품하나당 사이즈 -->
		...
		</a><br /><%=pdt.getPl_name() %></a><br /><%=pdt.getPl_price() %>원<br /><br /><br /><br />
	</td>
	<%
